...
Returns all cells whose pentagons intersect the great-circle segment between two points.

The result is uncompacted at the requested resolution and is not sorted — cells appear roughly in the order they were discovered along the segment.

```ts
function lineSegmentToCells(start: LonLat, end: LonLat, resolution: number): bigint[];
```

#### Parameters

- `start` **(LonLat)** Start point of the segment, as `[longitude, latitude]`
- `end` **(LonLat)** End point of the segment, as `[longitude, latitude]`
- `resolution` **(number)** Target resolution (0–30)

#### Return value

- **(bigint[])** Array of unique cell identifiers whose pentagons intersect the segment

#### Example

```ts
import { lineSegmentToCells } from 'a5-js';

const cells = lineSegmentToCells([2.3522, 48.8566], [-0.1276, 51.5074], 8); // Paris → London
```

### lineStringToCells

Returns all cells along a polyline defined by a sequence of waypoints. Consecutive waypoints are connected with great-circle arcs.
...
- **(bigint[])** Array of unique cell identifiers along the polyline. Cells at waypoint junctions are deduplicated.
