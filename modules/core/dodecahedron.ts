...
import { toCartesian, toSpherical, fromLonLat, toFace, toPolar } from "./coordinate-transforms";
...
function getFaceTriangle(polar: Polar): FaceTriangle {
  ...
  vec2.add(vEdgeMidpoint, vCorner1, vCorner2);
  vec2.scale(vEdgeMidpoint, vEdgeMidpoint, 0.5);
  ...
  let facePoints = [odd ? vCorner1 : vCorner2, vCenter, vEdgeMidpoint];
  const ia = VERTEX_ORDER[0];
  const ib = VERTEX_ORDER[odd ? 1 : 2];
  const ic = VERTEX_ORDER[odd ? 2 : 1];
...
export function unprojectDodecahedron(spherical: Spherical, originTransform: quat, originRotation: Radians, resolution: number): Polar {
  ...
  const projected: Face = [0, 0] as Face;
  forwardVector(unprojected, sphericalTriangle, faceTriangle, projected as any);
