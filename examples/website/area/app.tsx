...
const CellVisualization: React.FC<{
  ...
  const projectH3Cell = cell => {
    ...
    return boundary.map(point => {
      ...
      const cosc =
        Math.sin(centerLat) * Math.sin(lat) + Math.cos(centerLat) * Math.cos(lat) * Math.cos(lon - centerLon);

      // Project to plane
      const x = (Math.cos(lat) * Math.sin(lon - centerLon)) / cosc;
      const y =
        (Math.cos(centerLat) * Math.sin(lat) - Math.sin(centerLat) * Math.cos(lat) * Math.cos(lon - centerLon)) / cosc;
  ...
  const projectA5Cell = cell => {
    ...
    return vertices.map(point => {
      ...
      // Calculate the cosine of the angular distance from center
      const cosc =
        Math.sin(centerLatRad) * Math.sin(lat) + Math.cos(centerLatRad) * Math.cos(lat) * Math.cos(lon - centerLonRad);
  ...
  // Project both cells
  const smallestProjected = tilingSystem === 'h3' ? projectH3Cell(currentSmallest) : projectA5Cell(currentSmallest);

  const largestProjected = tilingSystem === 'h3' ? projectH3Cell(currentLargest) : projectA5Cell(currentLargest);
...
// Prepare H3 data
const h3Data = getRes0Cells()
  .map(cell => cellToChildren(cell, H3_RESOLUTION))
  .flat();
...
const App: React.FC<{isMobile?: boolean}> = ({isMobile = false}) => {
  ...
  const domain = Array.from(
    {length: N},
    (_, i) => h3AreaLimits[0] + (i * (h3AreaLimits[1] - h3AreaLimits[0])) / (N - 1)
  );
  const getFillColor = (d: any, info: any) =>
    colorContinuous({
  ...
  return (
    ...
    >
      ...
      <Legend areaLimits={areaLimits} h3AreaLimits={h3AreaLimits} />
    </div>
  );
};
