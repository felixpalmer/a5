...
// - places: Natural Earth populated places (name, country, lon/lat, population)
const DATASETS = {
...
WHERE cell IN (
  ...
  {
    label: 'Capital proximity',
    query: `-- How many capital cities are within 500km? A spherical
-- cap around each capital, summed per cell
SELECT cell, COUNT(*) AS value
FROM (
  SELECT unnest(a5_uncompact(
    a5_spherical_cap(a5_lonlat_to_cell(longitude, latitude, 8), 500000.0), 8)) AS cell
  FROM places
  WHERE capital
)
GROUP BY 1`
...
function autoPalette(field: ValueField, sql: string): PaletteName {
  ...
  // Proximity/density counts read naturally as heat intensity
  if (source.includes('capital') || source.includes('cities')) return 'Heat';
  return 'Viridis';
}
