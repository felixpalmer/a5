...
const App: React.FC = () => {
  ...
  const baseResolution = useMemo(
    () => zoomToResolution(Math.round(effectiveZoom)),
    [effectiveZoom],
  );
  const resolution =
    fixedResolution ?? Math.min(28, baseResolution + resOffset);
  ...
  const handleExport = useCallback(() => {
    ...
    const ring = waypoints.map((wp) => [
      Math.round(wp[0] * 1e6) / 1e6,
      Math.round(wp[1] * 1e6) / 1e6,
    ]);
  ...
  const layers = useMemo(
    () => [
      ...
      new ScatterplotLayer<WaypointData>({
        ...
        getFillColor: (d) =>
          d.index === 0 ? [0, 200, 255, 255] : [255, 140, 0, 255],
        getRadius: 6,
        radiusUnits: "pixels",
        filled: true,
        stroked: true,
      ...
      >
        <div style={{ marginBottom: 4 }}>
          ...
          {fixedResolution !== null && (
            <span style={{ color: "#888" }}> (fixed)</span>
          )}
        </div>
        <div style={{ marginBottom: 8 }}>
          <label>
            Detail: {["Coarse", "Normal", "Fine", "Finest"][resOffset]}
          </label>
          <input
            type="range"
            min={0}
          ...
          {isPolygon && !outlineOnly && compactedCells && (
            <span style={{ color: "#888" }}>
              {showCompacted
                ...
                : ` (${compactedCells.length} compacted)`}
            </span>
          )}
        </div>
        {isPolygon && (
        ...
        {countries.length > 0 && (
          <div
            style={{ marginTop: 8, borderTop: "1px solid #ddd", paddingTop: 8 }}
          >
