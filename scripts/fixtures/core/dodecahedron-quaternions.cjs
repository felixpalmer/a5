...
const validationTests = {
  ...
  allFinite: quaternionData.every(data => data.quaternion.every(component => Number.isFinite(component))),
  northPoleIdentity:
    quaternionData[0].quaternion[0] === 0 &&
    quaternionData[0].quaternion[1] === 0 &&
    quaternionData[0].quaternion[2] === 0 &&
    quaternionData[0].quaternion[3] === 1,
  southPoleCorrect:
    quaternionData[11].quaternion[0] === 0 &&
  ...
  faceCentersDistinct: facecenters.every((fc1, i) =>
    facecenters.every((fc2, j) => {
      if (i === j) return true;
      const distance = Math.sqrt((fc1[0] - fc2[0]) ** 2 + (fc1[1] - fc2[1]) ** 2 + (fc1[2] - fc2[2]) ** 2);
      return distance > 0.1;
    })
  )
};
